...
void tcp_in(buf_t* buf, uint8_t* src_ip) {
    ...
    */
    uint16_t received_checksum = tcp_hdr->checksum16;
    tcp_hdr->checksum16 = 0;
    uint8_t temp_src_ip[NET_IP_LEN];
    memcpy(temp_src_ip, src_ip, NET_IP_LEN);
    uint16_t calculated_checksum = tcp_checksum(buf, temp_src_ip, net_if_ip);
    tcp_hdr->checksum16 = received_checksum;
    ...
    */
    ...
    uint32_t seq_number = swap32(tcp_hdr->seq_number32);
    uint32_t get_seq = seq_number;
    uint32_t ack_number = swap32(tcp_hdr->ack_number32);
    uint16_t window = swap16(tcp_hdr->window_size16);
    size_t hdr_len = 4*(uint16_t)tcp_hdr->data_offset;
    ...
    */
    ...
    if (connect->state == TCP_LISTEN)
    {
        ...
        connect->unack_seq = connect->next_seq = 20086;
        connect->ack = seq_number + 1;
        connect->remote_win = swap16(tcp_hdr->window_size16);
        buf_init(connect->tx_buf, 0);
     ...
     case TCP_ESTABLISHED:
        ...
        */
        ...
        else
        {
            ...
            uint16_t len = tcp_write_to_buf(connect, &temp_buf);
            if (data_len > 0 || len > 0)
            {
                tcp_send(&temp_buf, connect, tcp_flags_ack);
            }
        }
